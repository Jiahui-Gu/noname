...
${error}`);
  ...
  videoContent = {
    ...
    over: function(content) {
      const { html, handcardPoptips: recordedHandcardPoptips } = typeof content === "string" ? { html: content, handcardPoptips: null } : content;
      ...
      if (recordedHandcardPoptips) {
        for (const [id, targetName, cardInfos] of recordedHandcardPoptips) {
          ...
          lib.poptip.add({
            ...
            dialog(dialog2) {
              dialog2.add(`${targetName}的手牌`);
              dialog2[hs.length ? "addSmall" : "addText"](hs.length ? hs : "（没有手牌）");
              return dialog2;
            }
          });
        }
