function startInstalledUpdate() {
  ...
  const repoDir = localStorage.getItem("noname_update_repo") || path.join(window.process.env.USERPROFILE, ".copilot", "repos", "noname");
  const launchLog = path.join(resourcesDir, "update-launch.log");
  const logFailure = (error) => {
    try {
      const message = error instanceof Error ? error.stack || error.message : String(error);
      fs.appendFileSync(launchLog, `${(/* @__PURE__ */ new Date()).toISOString()} ${message}
`);
    } catch {
    }
  };
