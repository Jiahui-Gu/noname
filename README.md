...
新构建的 Electron 安装版每次启动只检查一次 `main` 内容更新。兼容的游戏构建会在后台校验并下载；不在对局时立即刷新，正在对局时则在游戏结束后刷新，不会退出客户端。Electron 外壳或依赖发生变化时会提示下载新版安装包。更新安装失败或新版本无法启动时，客户端会保留或自动恢复上一份可运行版本。
